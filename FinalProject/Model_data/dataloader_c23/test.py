...
def retrieve_images(compression="c40"):
    ...
    for label, case in enumerate(cases):
        ...
        for t in video_types:
            ...
            all_images.sort()  # Sort images to ensure consistency
            video_dict = {}
            for image in all_images:
                video_number = get_video_number(image)
                if video_number not in video_dict:
                    video_dict[video_number] = []
                video_dict[video_number].append(image)
            
            for video_number, images in video_dict.items():
                random.shuffle(images)  # Shuffle images before selecting
                if t == "youtube":
                    images = images[:50]
                else:
                    images = images[:10]  # Limit to 10 images per video
